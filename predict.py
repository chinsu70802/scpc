...
def main(wav, ckpt, prominence):
    ...
    print('promonaence',peak_detection_params["prominence"])    
    # load data
    ...
    # run inference
    preds,z = model(audio)  # get scores
    ...
    print(preds)
    with open("lab_w4_prc_s2.lab", "w") as f:
        for id in range(1, len(preds)):
            f.write(f"{preds[id -1 ]}  {preds[id]} {id-1}\n")
